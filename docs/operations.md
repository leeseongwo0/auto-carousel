...
Timer last/next activation, failed unit, automation aggregate counters, stale lease, pending/ambiguous notification and Sheets operation을 관찰한다. secret, Telegram message, Sheet payload, Google 오류 원문은 log/receipt에 복사하지 않는다.
...
첫 설치는 release window 전에 빈 SQLite authority file과 세 canonical lock file을 `newsbot:newsbot`, `0600`으로 provision한다. switch 뒤의 `init-db`가 최초 schema/cutover를 만든다. 이미 007 cutover를 마친 호스트의 이후 release와 rollback은 아래와 같은 compatible forward switch만 사용하며 immutable SQLite history를 초기화하거나 backup으로 복원하지 않는다.
...
후속 compatible release 또는 rollback은 initial cutover를 다시 실행하지 않는다. 모든 timer/service를 drain하고 동일 quiescence·switch gate를 통과한 뒤 새 runtime manifest digest를 append-only activation으로 기록한다.
...
```
명령은 `/usr/local/bin/newsbot` symlink가 가리키는 versioned release의 `runtime-manifest.json` schema/source identity를 직접 읽고 SHA-256을 다시 계산한다. 전달값이 실제 stable runtime과 다르면 SQLite activation을 기록하지 않는다.

첫 설치의 DB/lock provision은 **release window 이전에만** 실행한다; subsequent post-007 release에서는 실행하거나 existing DB를 바꾸지 않는다. preview receipt의 SHA-256을 그대로 apply에 사용한다. `NEWSBOT_CALLBACK_ACTOR_ID`는 `NEWSBOT_APPROVER_USER_IDS` 안의 단 한 명으로 `/etc/newsbot/newsbot.env`에 반드시 설정한다.
...
enable 순서는 반드시 Telegram → Sheets → collection이다. 이후 candidate의 인간 `[제작]` 승인, one-job generation, 정확한 draft의 인간 `[시트 전달]` 승인, Sheets metadata/append와 no-resend behavior를 E2E로 확인한다.
...
rollback은 initial cutover 후 007-compatible이고 이전에 verified된 runtime release로 forward switch하는 것뿐이다. migration은 forward-only다. cursor/outbox, attempts, Sheets/Telegram history, approval/release audit, containment receipt를 delete/update/restore하지 않는다. 문제가 난 code/config는 corrective release, 새 cutover receipt와 immutable operator resolution으로 복구한다.
