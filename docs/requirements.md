...
fixture 수집, fixture `reconcile`, `rank`, scripted 승인은 credential-free다. `auth-telethon`만 interactive MTProto authorization을 수행하며, `collect-live`/`reconcile-live`는 Telethon/MTProto, `notify-candidates`/`notify-review`/`poll-approvals`는 Telegram Bot API, `openai_compatible` generation은 OpenAI-compatible API를 각각 명시적으로 사용한다. provider-free 명령은 provider를 만들지 않고 Telethon을 import하거나 네트워크를 열지 않는다. Bot API는 채널 수집을 대체하지 않는다.
...
생성은 선택된 job만 lease한다. 실패한 provider 호출은 `failed_recoverable` job으로 남고, 같은 candidate의 `generate-pending` 또는 `poll-approvals --process-generation`이 이를 다시 lease할 수 있다. provider 실패는 draft, fake fallback, approval, outbox를 만들지 않는다. material source edit는 현재 job, generation, callback을 supersede하고 새 선택을 요구한다.

정확한 current draft만 별도 review에서 승인, 재생성, 페이지 `+/-`, 6/24/72시간 연기 또는 거절한다. review 승인은 export outbox를 원자적으로 만든다.
...
Telegram 미리보기는 4096 UTF-16 code unit 이하로 분할한다. surrogate pair를 분리하지 않으며, 조각을 이어 붙이면 원문과 같다. Instagram 캡션은 export의 텍스트이며, Figma 편집 및 Instagram 게시에는 자동화가 없다.
...
SQLite `export_outbox`에는 JSON과 Markdown의 canonical bytes, SHA-256, 공통 `export_id`, 상태가 있다. 승인 transaction이 outbox intent를 원자적으로 만든다. materialization은 temporary file, fsync, atomic rename, digest 확인을 사용한다. mismatch는 파일을 보존하고 `corrupt` 상태로 남긴다. `ready` 쌍만 수동 Figma/Instagram handoff 대상이다.
## 평가 정책
